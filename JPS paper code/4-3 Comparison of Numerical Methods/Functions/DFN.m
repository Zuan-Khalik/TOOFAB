...
% 
% Inputs: 
%    - input_current: This field contains information about the current 
%      profile. This field can be provided either as a scalar value, an 
%      array which contains the current levels at each specified sample 
%      time, or as a function handle, which points to a function that takes
%      the output voltage, current, concentration and potentials, and the 
%      parameters as input and mainly provides the current as output. The 
%      latter form is especially useful when the battery is desired to be 
%      controlled in closed-loop. 
%    - tf specifies the simulation time in seconds
%    - init_cond: This field specifies the initial conditions of the cell. This field 
%      can be specified either as a scalar value or as a struct containing 
%      information on the desired initial conditions. When specified as a 
%      scalar value, the toolbox assumes steady-state conditions. Then, 
%      the value can be either chosen to represent an initial voltage or 
%      as an initial state-of-charge. To specify an initial
%      state-of-charge, a value between 0 and 1 must be chosen, 
%      representing the state-of-charge. When a value larger than 2 is
%      given for \texttt{init\_cond}, the toolbox assumes that an initial 
%      voltage is specified. 
%    - param: This is an optional input argument, given as a struct. Here, 
%      any desired non-default parameter fields of the configurable 
%      parameters can be specified. Configurable parameters that have not 
%      been specified will be assumed a certain default value. 
% 
% Outputs:
...
function out = DFN(input_current,tf,init_cond,varargin)
