%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Script to simulate Fig. 4 in [1] for the high-energy (HE) cell [2]. 
%
...
% grid_select = [40,30, 20,16,12,10,9,8,7,6, 5,4,3,2]; 
grid_base = {40, 40 ,40 , 40, 40};

for i = 1:4
    [p.nn,p.ns,p.np,p.nrp,p.nrn] = grid_base{:}; 
...
for i = 1:5
    for j = 1:length(grid_select)
        ...
        for k = 1:4
%             grid_loop = [50,50,50,50,50]; 
%             grid_loop(i) = grid_select(j); 
            [p.nn,p.ns,p.np,p.nrp,p.nrn] = grid_loop{i,j,k}{:}; 
            out = DFN(input_current,time,soc_init,p);
            V_temp2{k} = out.V; 
            sim_time_temp2(k) = out.solution_time;
%             rmserror{i}(j,k) = rms(V_base-V{i,j});
        end
        V_temp{j} = V_temp2{1}; 
...
save('Data/impact_discretization_HE','t','i_app','grid_base','grid_select','rmserror','sim_time_mean','sim_time_base','soc_init','time','V','V_base')